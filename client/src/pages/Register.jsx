    ...
    <div className='mt-20 flex justify-center items-center h-screen'>
      ...
      className='p-[1px] rounded-[20px] shadow-card items-center green-orange-gradient'>
        <div className='bg-tertiary rounded-[20px] py-4 px-8 min-h-[280px] flex justify-evenly items-center flex-col'>
          <form onSubmit={registerUser} className="mt-4 flex flex-col gap-8">
          <h3 className={styles.sectionHeadText}>Register</h3>
            <span className="text-white
            font-medium leading-normal">Name</span>
            <input type='text' placeholder='Put your name here!' value={data.name} onChange={(e) => setData({...data, name: e.target.value})} 
            ...
            <input type='email' placeholder='Your email goes here!' value={data.email} onChange={(e) => setData({...data, email: e.target.value})}
            className="w-[20rem] bg-tertiary py-2 px-2
            placeholder:text-secondary text-sm
            text-white outline-none font-medium border-b-2
            bg-clip-padding backdrop-filter backdrop-blur-sm bg-opacity-20
            hover:border-accent focus:border-accent"/>
            <span className="text-white
            font-medium leading-normal">Password</span>
            <input type='password' placeholder='Then your password here!' value={data.password} onChange={(e) => setData({...data, password: e.target.value})}
            className="w-[20rem] bg-tertiary py-2 px-2
            placeholder:text-secondary text-sm
            text-white outline-none font-medium border-b-2
            bg-clip-padding backdrop-filter backdrop-blur-sm bg-opacity-20
            ...
            <button type='submit' className='mx-auto bg-tertiary backdrop-opacity-30 
              ...
              md:py-2 md:px-8 md:w-fit'>Register</button>
          </form>
        </div>
      </motion.div>
    </div>
